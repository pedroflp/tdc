...
export default function HonorPage({ matchId, user }: { user?: UserDTO, matchId: string }) {
  ...
  return (
    <main className='max-w-[70%] m-auto space-y-8'>
      <div className='flex justify-between items-center mb-8'>
        <div className='max-w-[60%] space-y-4'>
          <h1 className='text-4xl font-bold'>Honre os jogadores da partida</h1>
          <p className='text-muted-foreground'>Vote nos jogadores que você deseja honrar como MVP, Refém ou Pedreiro, para atribuir (ou desatribuir) pontos extras aos somatórios de pontos individuais dos jogadores.</p>
        </div>
        {votingTimeLeft ? (
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger>
                <h2 className='text-6xl font-bold'>{formatSecondsInDateDifference(votingTimeLeft)}</h2>
              </TooltipTrigger>
